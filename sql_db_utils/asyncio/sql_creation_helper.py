...
async def database_exists(url):  # NOSONAR noqa
    url = make_url(url)
    ...
    try:
        if dialect_name == "postgresql":
            ...
            engine = create_async_engine(url)
            if database:
                return database == ":memory:" or _sqlite_file_exists(database)  # NOSONAR noqa
            else:
...
async def create_database(url, encoding="utf8", template=None):  # NOSONAR noqa
    url = make_url(url)
    database = url.database
    dialect_name = url.get_dialect().name
    ...
    if (dialect_name == "mssql" and dialect_driver in {"pymssql", "pyodbc"}) or (
        dialect_name == "postgresql" and dialect_driver in {"asyncpg", "pg8000", "psycopg2", "psycopg2cffi", "psycopg"}
    ):
...
async def drop_database(url):  # NOSONAR noqa
    url = make_url(url)
    database = url.database
    dialect_name = url.get_dialect().name
    dialect_driver = url.get_dialect().driver
