...
class UTDeclarativeGenerator(DeclarativeGenerator):
    ...
    def generate_base(self) -> None:
        self.base = Base(
            literal_imports=[LiteralImport("sql_db_utils.asyncio", "DeclarativeBaseClassFactory")],
            declarations=[
                f'{self.base_class_name} = DeclarativeBaseClassFactory("{self.raw_database}", "{self.schema}")'
            ],
